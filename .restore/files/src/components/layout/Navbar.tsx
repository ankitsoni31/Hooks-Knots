            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="relative px-4 py-2 text-sm font-medium text-[#1F2937]/80 hover:text-[#1F2937] transition-colors duration-300 group font-sans"
                >
                  {link.name}
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 w-0 bg-[#C89B3C] transition-all duration-300 group-hover:w-6" />
                </a>
              ))}
            </nav>
                ...
                <nav className="flex flex-col gap-1">
                  {navLinks.map((link, i) => (
                    <motion.a
                      ...
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="px-4 py-3 text-base font-medium text-[#1F2937] hover:text-[#C89B3C] hover:bg-[#F8F6F2] rounded-xl transition-all duration-200 font-sans"
                    >
                      {link.name}
                    </motion.a>
                  ))}
